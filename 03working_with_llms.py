...
def demo_message():
    ...
    print(f"Response from the Pirate: {response}")
    print(messages)
    ...
    response = model.invoke(messages)
    print(f"Follow-up response from the Pirate: {response}")

    print(messages)
    
def exercise_multi_model1(question: str, models: list[str])-> dict[str, str]:
    
    responses = {}
    
    for model_name in models:
        model = init_chat_model(model=model_name,   model_provider="azure_openai")
...
if __name__ == "__main__":
    ...
    exercise_multi_model1("Is earth is round?", ["gpt-4o-mini", "gpt-4.1-nano"])
